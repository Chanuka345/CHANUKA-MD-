...
async function connectToWA() {
    ...
    conn.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
            ...
            const up = `CHANUKA-MD connected successfully ✅\n\nPREFIX: ${prefix}`;
    ...
    conn.ev.on('messages.upsert', async (mek) => {
        ...
        events.commands.map(async (command) => {
            if (body && command.on === "body") {
                command.function(conn, mek, m, { from, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins });
...
app.get("/", (req, res) => {
    res.send("CHANUKA-MD bot started✅");
});
...
setTimeout(() => {
    connectToWA();
}, 4000);const isBotAdmins = isGroup ? groupAdmins.includes(botNumber2) : false
const isAdmins = isGroup ? groupAdmins.includes(sender) : false
const isReact = m.message.reactionMessage ? true : false
const reply = (teks) => {
conn.sendMessage(from, { text: teks }, { quoted: mek })
